# -*- coding: utf-8 -*-
from src.datasets.BasicPoiData import BasicPoiData
...
class SemPicPoiData(BasicPoiData):
    """ Separación TRAIN/DEV/TEST asegurando todos los usuarios y restaurantes en TRAIN"""

    def __init__(self, config):
        BasicPoiData.__init__(self, config=config)

    def __poi_to_train__(self, train, test):
        # Obtener la lista de pois de los dos conjuntos
        pois_train = set(train.restaurantId.unique())
        pois_test = set(test.restaurantId.unique())

        # Mirar cuales no están en train y si en test
        move_to_train = list(pois_test-pois_train)
    ...
    def __split_dataset__(self, rev):
        ...
        train, dev = self.__poi_to_train__(train, dev)
